...
const ValidationItem = ({ valid, text }: { valid: boolean; text: string }) => (
  <div className="flex items-center space-x-2 text-xs">
    {valid ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <X className="w-3.5 h-3.5 text-gray-400" />}
    <span className={valid ? 'text-emerald-400' : 'text-gray-400'}>{text}</span>
  </div>
);
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!isPasswordValid) { setError('Password does not meet requirements'); setShowValidation(true); return; }
    setLoading(true);
    try {
      if (isRegistering) {
    ...
    <div className="min-h-screen relative flex items-center p-6 md:p-16 lg:p-24 overflow-hidden text-white font-sans bg-[#111]">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-12 mt-10 xl:px-10">
        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}
          ...
          <h1 className="text-6xl md:text-7xl lg:text-[80px] font-bold leading-[1.1] mb-6 tracking-tight">WELCOME</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 pt-4">
            ...
            <div>
              ...
              <input type="password" value={credentials.password}
                onChange={e => { setCredentials({ ...credentials, password: e.target.value }); setShowValidation(true); }}
                className="w-full px-4 py-3 bg-white/10 text-white border border-white/20 rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-transparent outline-none transition-all placeholder-white/40"
                placeholder="••••••••" required />
              {showValidation && credentials.password && (
                <div className="mt-3 p-3 bg-black/40 border border-white/10 rounded-xl space-y-1.5 backdrop-blur-md">
            ...
            {error && (
              <div className={`p-3 border text-sm rounded-lg ${error.includes('successful') ? 'bg-green-500/20 border-green-500/50 text-green-100' : 'bg-red-500/20 border-red-500/50 text-red-100'}`}>
                {error}
              </div>
            )}

...
import { getHighRiskContainersList } from '../services/api';
...
const HighRiskContainers = () => {
  ...
  useEffect(() => {
    const fetchContainers = async () => {
      setLoading(true);
      try {
        const result = await getHighRiskContainersList(1, 50);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div>
          <h1 className="text-2xl font-bold text-[#1B2A4A] dark:text-slate-100 tracking-tight">High Risk Containers</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Containers flagged as HIGH or CRITICAL risk</p>
        </div>
      </motion.div>
